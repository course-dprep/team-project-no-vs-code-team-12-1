...
### 1.3 Repository Structure and Documentation

``` plaintext
├── README.md
├── makefile
├── updated-project.Rproj
├── .Rhistory
├── .RData
├── .gitignore
├── 1-docs
...
suppressMessages(suppressWarnings(install.packages("here", quiet = TRUE)))
suppressMessages(suppressWarnings(install.packages("rstudioapi", quiet = TRUE)))

# GENERAL PACKAGES
suppressMessages(suppressWarnings(install.packages(c("tidyverse", "googledrive", "readr", "dplyr"), quiet = TRUE)))

# R MARKDOWN RENDER
suppressMessages(suppressWarnings(install.packages("reticulate", quiet = TRUE)))
suppressMessages(suppressWarnings(install.packages("knitr", quiet = TRUE)))

# ANALYSIS PACKAGES
suppressMessages(suppressWarnings(install.packages("ggcorrplot", quiet = TRUE)))
suppressMessages(suppressWarnings(install.packages("sjPlot", quiet = TRUE)))
suppressMessages(suppressWarnings(install.packages("sjmisc", quiet = TRUE)))

# Install the gridExtra package
suppressMessages(suppressWarnings(install.packages("gridExtra", quiet = TRUE)))
```
...
# View the first few rows of business_data
kable(head(business_data), caption = "First Few Rows of Business Data")
```

**user data**

```{r, eval=TRUE, echo=FALSE,results='markup'}
# View the first few rows of user_data
kable(head(user_data), caption = "First Few Rows of User Data")
```

**review data**

```{r, eval=TRUE, echo=FALSE,results='markup'}
# View the first few rows of review_data
kable(head(review_data), caption = "First Few Rows of Review Data")
```
...
```{r, eval=TRUE, echo=FALSE,results='markup'}
# Displaying the first few rows of the selected business_data
kable(head(business_data), caption = "First Few Rows of Selected Business Data")
```

```{r, eval=TRUE, echo=FALSE,results='markup'}
# Displaying the first few rows of the selected user_data
kable(head(user_data), caption = "First Few Rows of Selected User Data")
```

```{r, eval=TRUE, echo=FALSE,results='markup'}
# Displaying the first few rows of the selected review_data
kable(head(review_data), caption = "First Few Rows of Selected Review Data")
```
...
To **prepare the new data set** for subsequent use, we **transformed the data** by **renaming the columns** to have **meaningful and easily traceable names**.
...
![Figure 1](images/distribution_ratings_takeout_by_elite_review.png){fig-align="center"}
...
![Figure 2](images/distribution_user_ratings_by_elite_review.png){fig-align="center"}
...
![Figure 3](images/user_ratings_by_region_elite_review.png){fig-align="center"}
...
![Figure 4](images/distribution_review_count_business_by_is_open.png){fig-align="center"}
...
![Figure 5](images/avg_rating_gt_1000_by_region.png){fig-align="center"}
...
![Figure 6](images/avg_rating_gt_1000_by_is_open.png){fig-align="center"}
...
avg_rating_lt_1000_region_plot <- ggplot(data_lt_1000_region, aes(x = reorder(region, -avg_rating), y = avg_rating, fill = elite_review)) +
  geom_col(position = "dodge") +
  coord_flip() +  labs(title = "Average User Rating of Businesses with Review Count < 1000 by Region and Elite Status",
       x = "Region", y = "Average User Rating", fill = "Elite Status") +
  theme(plot.title = element_text(hjust = 0.5))
```

![Figure 7](images/avg_rating_lt_1000_by_region.png){fig-align="center"}
...
![Figure 8](images/avg_rating_lt_1000_by_is_open.png){alt="Figure 8" fig-align="center"}
...
![Figure 9](images/relationship_fans_user_rating_faceted_by_region.png){fig-align="center"}

...
### 2.1.3 Download Yelp Data directly from Google Drive
...
Particularly, the **googledrive** package enables direct interaction with Google Drive, allowing for seamless downloading of files, while **tidyverse** provides essential tools for data manipulation and analysis. Automating the download process saves time and minimizes manual errors during setup, allowing you to focus on the analysis.

------------------------------------------------------------------------

### 2.1.4 Inspect Yelp Data
...
The business data set consists of the following variables:

<<<<<<< HEAD:1. docs/Group Report.qmd
...
| hours | *An object of key day-to-value hours, hours are using a 24-hr clock* | Character |
=======
| Variable     | Description                                                          | **Data class** |
|--------------|---------------------------------------------|--------------|
| business_id  | *A unique character string for each business*                        | Character      |
...
| categories   | *An array of different business categories*                          | Character      |
| hours        | *An object of key day-to-value hours, hours are using a 24-hr clock* | Character      |
>>>>>>> 2cfdcf77804fb90ca496c36f6fc84b16c1c4dcf5:1-docs/Group Report.qmd
...
The user dataset consists of the following variables:
...
The different variables are evaluated and a selection of variables is chosen to keep in the data set to help answer the research question. These variables in the review data set are:
...
The first step of preparing the data is loading it into the required R libraries.
...
**Saving the merged data set**

Because we are working with large data files that take a long time to process, it is important to save and create csv files while preparing the code. This is done below with the `write_csv()` function.
...
Because of the large size of the merged data, with almost seven million observations, a sample of 50.000 observations is taken. This sample will be cleaned in the next phase.

To take make sure each sample that will be taken from the data set is identical, the seed is set to 90 using `set.seed()`.
...
The first step of cleaning the data is loading it into the required R libraries.
...
**Inspecting variable names in the data set**
...
After running the code, it became clear that two of the variables were similar in different data sets. These were the "**stars**" and "**review_count**" variables. These variables were differentiated with a "**.x**" and "**.y**" at the end. In addition, the "**name**" variable seemed unclear about what its name indicated.
...
To check if there are any NA's in one of the variables the "**variable_na**" data set is created as an overview of which variables include NA values.
...
The "**variable_na**" table shows that "**elite**" contains 37669 NA values and that "**attributes**" contains 1331 NA values.

To make it easier for analysis, we decided to encode the values in the "**elite**" column by creating a new column 'elite_label_encoder' to identify influential users. The logic is - If the 'elite' column is "NA" or an empty string, set 'elite_label_encoder' to 0 (not influential user) - If the 'elite' column has a value, set 'elite_label_encoder' to 1 (influential user).
...
Rather than analyzing each state and city individually, we **created a new variable called 'state_divisions'** which groups states into broader divisions. This approach helps **reduce noise and streamline the data set**, making it more manageable and logical to work with.
...
Before dummy encoding the 'attributes' column to extract the 'RestaurantsTakeOut' value, we first address the NA values by removing them. Then, we use `str_detect` to identify the desired attribute and encode it as needed (1 for take-out restaurants, 0 for non-take-out restaurants).
...
**Save the finalized sample data**

Finally, we save the data, which will be used for further analysis.

```{r, echo=TRUE, eval=FALSE}
write_csv(takeout_data, "takeout_data.csv")
```
